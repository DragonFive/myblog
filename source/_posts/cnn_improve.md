...
定义squeeze层中1x1卷积核的数量是$s_{1*1}$，类似的，expand层中1x1卷积核的数量是$e_{1*1}$， 3x3卷积核的数量是$e_3*3$。令$s_{1x1} < e_{1x1}+ e_{3x3}$从而保证输入到3x3的输入通道数减小。SqueezeNet的网络结构由若干个 fire module 组成
